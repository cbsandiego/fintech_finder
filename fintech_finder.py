...
st.markdown("# Fintech Finder Web Application")
...
st.sidebar.write(f"Client's Eth Account Address: {account.address}")
...
get_balance = get_balance(w3, account.address)
st.sidebar.write(f"Client's Eth Balance: {get_balance}")
...
st.sidebar.write(f"Candidate's hourly rate: {hourly_rate}")

# Identify the FinTech Finder candidate's Ethereum Address
candidate_address = candidate_database[person][1]

# Write the FinTech Finder candidate's Ethereum Address to the sidebar
st.sidebar.write(f"Candidate's Ethereum Address: {candidate_address}")
